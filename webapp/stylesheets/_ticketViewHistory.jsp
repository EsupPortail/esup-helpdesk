...
<e:dataTable columnClasses="colCenter,colLeftMax" id="actionData" width="100%" 
	...
	<t:column >
		...
		<t:graphicImage value="/media/images/protected.png" style="#{he.canChangeScope ? 'cursor: pointer' : ''}" rendered="#{he.action.scope == 'INVITED_MANAGER'}" onclick="javascript:{showHideElement('viewTicketForm:actionData:'+#{variable}+':editScope');}" />
	...
	<t:column >
		...
		<h:panelGroup rendered="#{he.canUpdateInformation}">
			<e:bold value=" " />
			<t:graphicImage value="/media/images/edit.png" style="cursor: pointer" 
				 onclick="simulateLinkClick('viewTicketForm:actionData:'+#{variable}+':updateInformation');" />
		</h:panelGroup>
		<e:commandButton 
